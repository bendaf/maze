...
public class MazeSerializer {
	public Maze deserializeMaze(BufferedReader mazeReader) throws IOException {
		...
		while ((line = mazeReader.readLine()) != null) {
			...
			Square newSquare = new Square(getWall(splitted[2]), getWall(splitted[3]),
					getWall(splitted[4]), getWall(splitted[5]), getObject(splitted[6]));
	...
	private Wall getWall(String wallType){
		if(wallType.equals("normal"))
			return new Wall();
	...
	private SquareObject getObject(String objectType){
		if(objectType.equals("start")){
			return new Start();
		}else if(objectType.equals("end")){
			return new End();
